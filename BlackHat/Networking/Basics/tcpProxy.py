...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
    ...
    # loop, read from local, sent to remote, send to local
    while True:
        # read from local host
        local_buffer = receive_from(client_socket)

        if len(local_buffer):
            print("[==>] Received %d bytes form localhost"
                  % len(local_buffer))
            ...
            # close connection when there's no more data
...
def receive_from(connection):
    buffer = ""
    # 2 second timeout; adjust depending on target
    connection.settimeout(2)

    try:
        # keep reading buffer until no more data or timeout
        while True:
...
def request_handler(buffer):
    # perform packet modications
    return buffer

# modify any responses destined for local host
...
def main():
        if len(sys.argv[1:]) != 5:
            ...
            print("Example: ./proxy.py 172.0.0.1 9000 10.12.132.1 9000 True")
            sys.exit(0)
        ...
        # tell proxy to connect & receive data
        # prior to sending to remote host
